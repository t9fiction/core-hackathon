...
const PublicTokenListing: React.FC<PublicTokenListingProps> = ({ onSelectToken }) => {
  ...
  // Initialize token details with addresses when allTokens changes
  useEffect(() => {
    ...
    setTokenDetails(initialDetails);
  }, [allTokens]);
  ...
  // Update token data when individual cards fetch their details
  const handleTokenDataUpdate = (address: string, data: Partial<TokenInfo>) => {
    setTokenDetails(prevDetails => 
      prevDetails.map(token => 
  ...
  // Set loading to false once we have some real data loaded
  useEffect(() => {
    if (tokenDetails.length > 0) {
      const hasRealData = tokenDetails.some(token => 
        token.name !== 'Loading...' && token.symbol !== 'Loading...'
      );
      if (hasRealData && isLoadingDetails) {
        setIsLoadingDetails(false);
      }
    }
  }, [tokenDetails, isLoadingDetails]);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredTokens.map((token) => (
            <TokenCard
              ...
              onDataUpdate={handleTokenDataUpdate}
            />
          ))}
        </div>
      )}
...
interface TokenCardProps {
  token: TokenInfo;
  onSelect: () => void;
  onDataUpdate: (address: string, data: Partial<TokenInfo>) => void;
}

const TokenCard: React.FC<TokenCardProps> = ({ token, onSelect, onDataUpdate }) => {
  const chainId = useChainId();
  const contractAddresses = getContractAddresses(chainId);
  const WETH_ADDRESS = "0xfff9976782d46cc05630d1f6ebab18b2324d6b14";
  ...
  // Update parent state when token data is fetched
  useEffect(() => {
    if (tokenName && tokenName !== token.name) {
      onDataUpdate(token.address, { name: tokenName as string });
    }
  }, [tokenName, token.address, token.name, onDataUpdate]);

  useEffect(() => {
    if (tokenSymbol && tokenSymbol !== token.symbol) {
      onDataUpdate(token.address, { symbol: tokenSymbol as string });
    }
  }, [tokenSymbol, token.address, token.symbol, onDataUpdate]);

  useEffect(() => {
    if (totalSupply) {
      const formattedSupply = formatEther(totalSupply as bigint);
      if (formattedSupply !== token.totalSupply) {
        onDataUpdate(token.address, { totalSupply: formattedSupply });
      }
    }
  }, [totalSupply, token.address, token.totalSupply, onDataUpdate]);

  useEffect(() => {
    if (tokenInfo && creator !== token.creator) {
      onDataUpdate(token.address, { 
        creator: creator as string,
        deploymentTime: deploymentTime as bigint
      });
    }
  }, [tokenInfo, creator, deploymentTime, token.address, token.creator, onDataUpdate]);

  useEffect(() => {
    if (poolInfo && hasPool !== token.hasPool) {
      onDataUpdate(token.address, { hasPool: hasPool as boolean });
    }
  }, [poolInfo, hasPool, token.address, token.hasPool, onDataUpdate]);
