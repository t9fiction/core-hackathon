...
describe("PumpFunToken (Simplified)", function () {
    let pumpFunToken;
    ...
    describe("Deployment", function () {
        ...
        it("Should emit TokenCreated event", async function () {
            ...
            const newToken = await PumpFunToken.deploy(tokenName, tokenSymbol, totalSupply, owner.address);
    ...
    describe("Deployment Validation", function () {
        const PumpFunToken = ethers.getContractFactory("PumpFunToken");

        it("Should revert with empty name", async function () {
            const factory = await PumpFunToken;
            await expect(
                factory.deploy("", tokenSymbol, totalSupply, owner.address)
            ).to.be.revertedWithCustomError(factory, "PumpFunToken__InvalidName");
        });
        ...
        it("Should revert with empty symbol", async function () {
            ...
            ).to.be.revertedWithCustomError(factory, "PumpFunToken__InvalidSymbol");
        });
        ...
        it("Should revert with zero total supply", async function () {
            ...
            ).to.be.revertedWithCustomError(factory, "PumpFunToken__ZeroAmount");
        });
    ...
    describe("Edge Cases", function () {
        it("Should handle maximum token supply correctly", async function () {
            ...
            const PumpFunToken = await ethers.getContractFactory("PumpFunToken");
            const maxToken = await PumpFunToken.deploy("Max Token", "MAX", maxSupply, owner.address);
        ...
        it("Should handle minimum token supply correctly", async function () {
            ...
            const PumpFunToken = await ethers.getContractFactory("PumpFunToken");
            const minToken = await PumpFunToken.deploy("Min Token", "MIN", minSupply, owner.address);
