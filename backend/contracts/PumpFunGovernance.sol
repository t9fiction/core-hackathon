...
contract PumpFunGovernance is Ownable, ReentrancyGuard {
    // Custom Errors
    error PumpFunGovernance__ProposalNotFound();
    error PumpFunGovernance__ProposalNotActive();
    error PumpFunGovernance__ProposalAlreadyExecuted();
    error PumpFunGovernance__ProposalNotPassed();
    error PumpFunGovernance__AlreadyVoted();
    error PumpFunGovernance__InsufficientTokenBalance();
    error PumpFunGovernance__InvalidProposalType();
    error PumpFunGovernance__InvalidRecipients();
    error PumpFunGovernance__EmptyDescription();
    error PumpFunGovernance__ExecutionFailed();
    ...
    function createProposal(
        ...
        if (bytes(description).length == 0) revert PumpFunGovernance__EmptyDescription();
        if (proposalType == 0 || proposalType > 4) revert PumpFunGovernance__InvalidProposalType();
        
        // Check if user has minimum token balance to create proposal
        uint256 creatorBalance = IERC20(token).balanceOf(msg.sender);
        if (creatorBalance < MIN_VOTING_POWER) revert PumpFunGovernance__InsufficientTokenBalance();
        ...
        if (proposalType == 4) { // Airdrop proposal
            if (recipients.length == 0 || recipients.length != amounts.length) {
                revert PumpFunGovernance__InvalidRecipients();
            }
        }
    ...
    function vote(uint256 proposalId, bool support) external nonReentrant {
        if (proposalId == 0 || proposalId > proposalCount) revert PumpFunGovernance__ProposalNotFound();
        
        Proposal storage proposal = proposals[proposalId];
        
        if (!proposal.active) revert PumpFunGovernance__ProposalNotActive();
        if (block.timestamp > proposal.endTime) revert PumpFunGovernance__ProposalNotActive();
        if (hasVotedOnProposal[proposalId][msg.sender]) revert PumpFunGovernance__AlreadyVoted();

        // Get voter's token balance as voting power
        uint256 votingPower = IERC20(proposal.token).balanceOf(msg.sender);
        if (votingPower == 0) revert PumpFunGovernance__InsufficientTokenBalance();
    ...
    function executeProposal(uint256 proposalId) external nonReentrant {
        ...
        if (block.timestamp <= proposal.endTime) revert PumpFunGovernance__ProposalNotActive();
        if (proposal.votesFor <= proposal.votesAgainst) revert PumpFunGovernance__ProposalNotPassed();
    ...
    function _executeTokenFunction(address token, string memory functionName, uint256 value) internal {
        ...
        try PumpFunToken(token).owner() returns (address tokenOwner) {
