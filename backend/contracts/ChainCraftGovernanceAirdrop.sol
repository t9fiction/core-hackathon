...
contract ChainCraftGovernanceAirdrop is Ownable, ReentrancyGuard {
    // Custom Errors
    error ChainCraftGovernanceAirdrop__InvalidProof();
    error ChainCraftGovernanceAirdrop__AlreadyClaimed();
    error ChainCraftGovernanceAirdrop__InsufficientBalance();
    error ChainCraftGovernanceAirdrop__TransferFailed();
    error ChainCraftGovernanceAirdrop__InvalidMerkleRoot();
    error ChainCraftGovernanceAirdrop__AirdropNotActive();
    error ChainCraftGovernanceAirdrop__InvalidAmount();
    ...
    function configureAirdrop(
        ...
        if (merkleRoot == bytes32(0)) revert ChainCraftGovernanceAirdrop__InvalidMerkleRoot();
        if (totalAmount == 0) revert ChainCraftGovernanceAirdrop__InvalidAmount();

        // Check that contract has sufficient token balance
        uint256 contractBalance = IERC20(token).balanceOf(address(this));
        if (contractBalance < totalAmount) revert ChainCraftGovernanceAirdrop__InsufficientBalance();
    ...
    function claimAirdrop(
        ...
        if (!config.active) revert ChainCraftGovernanceAirdrop__AirdropNotActive();
        if (block.timestamp < config.startTime || block.timestamp > config.endTime) {
            revert ChainCraftGovernanceAirdrop__AirdropNotActive();
        }
        if (hasClaimed[token][msg.sender]) revert ChainCraftGovernanceAirdrop__AlreadyClaimed();

        // Verify merkle proof
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, amount));
        if (!MerkleProof.verify(merkleProof, config.merkleRoot, leaf)) {
            revert ChainCraftGovernanceAirdrop__InvalidProof();
        }

        // Check if there are enough tokens remaining
        if (config.claimedAmount + amount > config.totalAmount) {
            revert ChainCraftGovernanceAirdrop__InsufficientBalance();
        }
        ...
        if (!success) revert ChainCraftGovernanceAirdrop__TransferFailed();
    ...
    function batchClaimAirdrop(
        ...
        if (recipients.length != amounts.length || recipients.length != merkleProofs.length) {
            revert ChainCraftGovernanceAirdrop__InvalidAmount();
        }
    ...
    function updateMerkleRoot(address token, bytes32 newMerkleRoot) external onlyOwner {
        if (newMerkleRoot == bytes32(0)) revert ChainCraftGovernanceAirdrop__InvalidMerkleRoot();
    ...
    function emergencyWithdraw(address token, address to, uint256 amount) external onlyOwner {
        if (amount == 0) revert ChainCraftGovernanceAirdrop__InvalidAmount();
        
        uint256 contractBalance = IERC20(token).balanceOf(address(this));
        if (contractBalance < amount) revert ChainCraftGovernanceAirdrop__InsufficientBalance();
