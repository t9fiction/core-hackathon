 ...
 * @dev Manages DEX integration for ChainCraft tokens with automated liquidity provision using Uniswap V3
 */
contract ChainCraftDEXManager is Ownable, ReentrancyGuard {
    ...
    error ChainCraftDEXManager__InsufficientLiquidity();
    error ChainCraftDEXManager__PairAlreadyExists();
    error ChainCraftDEXManager__SlippageExceeded();
    error ChainCraftDEXManager__DeadlineExpired();
    error ChainCraftDEXManager__UnauthorizedToken();
    error ChainCraftDEXManager__InvalidFactoryAddress();
    error ChainCraftDEXManager__InsufficientBalance(uint256 available, uint256 requested);
    error ChainCraftDEXManager__InvalidPathLength();
    error ChainCraftDEXManager__PathFeesLengthMismatch();
    error ChainCraftDEXManager__TransferFailed();
    error ChainCraftDEXManager__InvalidFeeTier();
    error ChainCraftDEXManager__PoolDoesNotExist();
    error ChainCraftDEXManager__QuoterFailed(string reason);
    ...
    function createLiquidityPoolWithETH(address token, uint24 fee, uint256 tokenAmount) external payable nonReentrant {
        if (!authorizedTokens[token]) revert ChainCraftDEXManager__UnauthorizedToken();
        if (tokenAmount == 0) revert ChainCraftDEXManager__InvalidAmount();

        address token0 = token < WETH ? token : WETH;
        address token1 = token < WETH ? WETH : token;
        if (tokenPools[token0][token1][fee].isActive) revert ChainCraftDEXManager__PairAlreadyExists();
    ...
    function createLiquidityPool(address tokenA, address tokenB, uint24 fee, uint256 amountA, uint256 amountB)
        ...
        nonReentrant
    {
        if (tokenA == address(0) || tokenB == address(0)) revert ChainCraftDEXManager__InvalidTokenAddress();
        if (!authorizedTokens[tokenA] && !authorizedTokens[tokenB]) revert ChainCraftDEXManager__UnauthorizedToken();
        if (amountA == 0 || amountB == 0) revert ChainCraftDEXManager__InvalidAmount();
        ...
        nonReentrant
    {
        ...
        if (!tokenPools[orderedToken0][orderedToken1][fee].isActive) revert ChainCraftDEXManager__PoolDoesNotExist();
        if (tokenAmount0 == 0 || tokenAmount1 == 0) revert ChainCraftDEXManager__InvalidAmount();

        // Handle ETH/WETH conversion
        if (token0 == WETH && msg.value > 0) {
            if (msg.value != tokenAmount0) revert ChainCraftDEXManager__InvalidAmount();
            IWETH(WETH).deposit{value: msg.value}();
            IERC20(token1).transferFrom(msg.sender, address(this), tokenAmount1);
        } else if (token1 == WETH && msg.value > 0) {
            if (msg.value != tokenAmount1) revert ChainCraftDEXManager__InvalidAmount();
            IWETH(WETH).deposit{value: msg.value}();
    ...
    function swapExactETHForTokens(address tokenOut, uint24 fee) external payable nonReentrant {
        ...
        try quoterV2.quoteExactInputSingle(params) returns (uint256 amountOut, uint160, uint32, uint256) {
            ...
            revert ChainCraftDEXManager__InsufficientLiquidity();
        }
    ...
    {
        if (!authorizedTokens[tokenOut]) revert ChainCraftDEXManager__UnauthorizedToken();
        if (slippageTolerance > 5000) revert ChainCraftDEXManager__SlippageExceeded();
        if (fee != 100 && fee != 500 && fee != 3000 && fee != 10000) revert ChainCraftDEXManager__InvalidFeeTier();
        ...
        address poolAddress = uniswapV3Factory.getPool(token0, token1, fee);
        if (poolAddress == address(0)) revert ChainCraftDEXManager__PoolDoesNotExist();
        ...
        try quoterV2.quoteExactInputSingle(params) returns (uint256 amountOut, uint160, uint32, uint256) {
            ...
            revert ChainCraftDEXManager__QuoterFailed(reason);
        } catch {
            revert ChainCraftDEXManager__QuoterFailed("Unknown error in QuoterV2");
        }
    ...
    function swapExactTokensForETH(address tokenIn, uint24 fee, uint256 amountIn) external nonReentrant {
        ...
        if (IERC20(tokenIn).balanceOf(msg.sender) < amountIn) {
            revert ChainCraftDEXManager__InsufficientBalance(IERC20(tokenIn).balanceOf(msg.sender), amountIn);
        }
        ...
        nonReentrant
    {
        ...
        if (userBalance < amountIn) {
            revert ChainCraftDEXManager__InsufficientBalance(userBalance, amountIn);
        }
        uint256 allowance = IERC20(tokenIn).allowance(msg.sender, address(this));
        if (allowance < amountIn) {
            revert ChainCraftDEXManager__InsufficientBalance(allowance, amountIn); // Consider a custom error for allowance
        }
    ...
    {
        if (path.length < 2) revert ChainCraftDEXManager__InvalidPathLength();
        if (path.length - 1 != fees.length) revert ChainCraftDEXManager__PathFeesLengthMismatch();

        for (uint256 i = 0; i < path.length; i++) {
            if (!authorizedTokens[path[i]] && path[i] != WETH) {
                revert ChainCraftDEXManager__UnauthorizedToken();
            }
        }
    ...
    {
        if (tokenIn == address(0) || tokenOut == address(0)) revert ChainCraftDEXManager__InvalidTokenAddress();
        if (amountIn == 0) revert ChainCraftDEXManager__InvalidAmount();
    ...
    function testQuoteExactInputSingle(
        ...
        if (fee != 100 && fee != 500 && fee != 3000 && fee != 10000) {
            revert ChainCraftDEXManager__InvalidFeeTier();
        }
        ...
        address poolAddress = uniswapV3Factory.getPool(orderedToken0, orderedToken1, fee);
        if (poolAddress == address(0)) {
            revert ChainCraftDEXManager__PoolDoesNotExist();
        }
